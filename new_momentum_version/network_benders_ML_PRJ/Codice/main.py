...
for i in [.2]:
    l1 = Layer(inputs=17,weights=[],sorta=logistic,derivata=derivata_logistic,num_unit=7)
    l3 = Output_Layer(inputs=7,weights=[],sorta=logistic,derivata=derivata_logistic,num_unit=1)
    net_monk = Net([l1,l3])
    a,b,c,d = net_monk.fit(l_train.x, l_train.y, eta=0.35,momentum=0.8, mode='online', epochs=100,hold_out=i)
