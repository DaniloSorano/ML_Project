...
for i in [.2]:
    l1 = Layer(inputs=10,weights=[],sorta=logistic,derivata=derivata_logistic,num_unit=4,fun_in=True)
    l3 = Output_Layer(inputs=4,weights=[],sorta=identity,derivata=derivata_identity,num_unit=2)
    net_cup = Net([l1,l3])

    a,b,c,d = net_cup.fit(l_train.x, l_train.y, eta=0.5,momentum=0.6,lamb=0.01, mode='minibatch', epochs=300,batch_size=50,hold_out=i) # anche questo non male
