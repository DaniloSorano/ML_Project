            ...
            for momentum in range(2,7):
                ...
                if mode == 'minibatch':
                    for batch_size in [10,30,50,100]:
                        ...
                        loss,acc,val_loss,val_acc=n.fit(l_train.x, l_train.y, eta=eta/10., mode=mode, epochs=1, momentum=momentum/10.,batch_size=batch_size)
                        predicted = []
                        ...
                        n.save_conf(config)
                        n.save_score(config,loss,acc,val_loss,val_acc,test_acc)
                        config=config+1
                else:
                    print n.name+' config=',str(config)
                    loss,acc,val_loss,val_acc=n.fit(l_train.x, l_train.y, eta=eta/10., mode=mode, epochs=5, momentum=momentum/10.)
                    predicted = []
                    for i,p in enumerate(l_test.x):
                        hx = n.predict(p)
                        predicted.append(hx)
