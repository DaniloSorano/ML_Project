                        ...
                        if task == "monks-3":
                            for lamb in [0.0, 0.01, 0.02]:
                                ...
                                for tries in range(0,5):
                                    ...
                                    loss,acc,val_loss,val_acc=n.fit(l_train.x, l_train.y, eta=eta/10., mode=mode, epochs=500, momentum=momentum/10.,batch_size=batch_size,early=early, lamb=lamb, hold_out=0.2)
                                    predicted = []
                            ...
                            for tries in range(0, 5):
                                # validation_accuracy_for_topology=[]
                                n = pkl.load(open(folder + 'Net_' + str(topology) + '_try_' + str(tries), 'rb'))
                                print str((n.name.split('_'))), eta, momentum, mode, (
                                    batch_size if mode == 'minibatch' else ''), str(teta)

                                loss, acc, val_loss, val_acc = n.fit(l_train.x, l_train.y, eta=eta / 10., mode=mode,
                                                                     epochs=500, momentum=momentum / 10.,
                                                                     batch_size=batch_size, early=early, hold_out=0.2)
                                predicted = []
                                ...
                                print 'MSE_val = ', val_loss[-1], 'Validation Accuracy = ', val_acc[
                                    -1], 'Test Accuracy = ', test_acc
                    ...
                    if task == "monks-3":
                        for lamb in [0.0, 0.01, 0.02]:
                            ...
                            for tries in range(0,5):
                                ...
                                loss,acc,val_loss,val_acc=n.fit(l_train.x, l_train.y, eta=eta/10., mode=mode, epochs=500, momentum=momentum/10., lamb=lamb, early=early, hold_out=0.2)
                                predicted = []
                                for i,p in enumerate(l_test.x):
                                    hx = n.predict(p)
                                    predicted.append(hx)
                        ...
                        for tries in range(0, 5):
                            ...
                            loss, acc, val_loss, val_acc = n.fit(l_train.x, l_train.y, eta=eta / 10., mode=mode,
                                                                 epochs=500, momentum=momentum / 10., early=early,
                                                                 hold_out=0.2)
                            predicted = []
                            for i, p in enumerate(l_test.x):
                                hx = n.predict(p)
